...
def eeg_session_analysis():
    df = csv_to_dataframe("data\\sessions", "subject_1.csv")
    ...
    def show_raw_visualization(data):
        ...
        plt.tight_layout()
        plt.savefig('images\\parameters_plot.png')
        plt.show()
        plt.close()
    ...
    plt.savefig('images\\raw_parameters.png')
    plt.show()
    plt.close()
    ...
    # plt.title(f"Training and validation dataset: {titles[prediction_index]}")
    # plt.plot(all_data[prediction_index], label="All data", linestyle="-", c=colors[6])
    ...
    #          c=colors[5])
    # plt.legend()
    # plt.savefig('images\\training_validation_dataset.png')
    # plt.show()
    # plt.close()
